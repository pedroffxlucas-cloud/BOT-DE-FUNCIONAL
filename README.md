...
- `FUNCTIONAL_ROLES`: cargos do menu `Escolher cargo`, no formato `AGENTE DHPP:id,AGENTE GER:id,AGENTE PCESP:id`.
...
Para o botão `Escolher cargo` funcionar, configure `FUNCTIONAL_ROLES` no Render. O cargo do bot precisa estar acima dos cargos que ele vai setar e o bot precisa da permissão `Gerenciar Cargos`.
